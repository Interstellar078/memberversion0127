...
export const ItineraryTable: React.FC<ItineraryTableProps> = ({
    ...
    setIsChatOpen, handleRefreshCosts, handleDeleteRow, handleDragEnd, updateRow,
    ...
    const Th = (id: string, label: string | React.ReactNode, bgClass = '', textClass = 'text-slate-600', sticky = false) => {
        ...
        return (
            <th key={id} style={{ width: w, minWidth: w }} className={`px-3 py-4 text-left text-xs font-bold uppercase tracking-wider group ${textClass} ${bgClass} sticky top-0 ${sticky ? 'left-0 z-30 shadow-[4px_0_12px_rgba(0,0,0,0.05)] bg-slate-50' : 'z-20 bg-slate-50'} border-b border-gray-200`}>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left border-collapse">
                            ...
                            <Droppable droppableId="day-rows">
                                {(provided) => (
                                    <tbody className="divide-y divide-gray-100" {...provided.droppableProps} ref={provided.innerRef}>
                                        {rows.map((row, index) => {
                                            ...
                                            return (
                                                // @ts-ignore - key is required by React but DragabbleProps type definition might be missing it in this version
                                                <Draggable key={row.id} draggableId={row.id} index={index}>
                                                    {(provided, snapshot) => (
                                                        <tr
                                                            ref={provided.innerRef}
                                                            {...provided.draggableProps}
                                                            className={`group transition-all duration-200 border-b border-gray-100 last:border-0 ${snapshot.isDragging ? 'bg-blue-50 shadow-lg relative z-20' : 'hover:bg-blue-50/30'}`}
                                                        >
                                                            <td className="p-3 sticky left-0 bg-white group-hover:bg-blue-50/40 transition-colors z-10 font-bold text-center text-slate-400 shadow-[4px_0_12px_rgba(0,0,0,0.02)]">
                                                            ...
                                                            <td className="p-2"><input type="date" className="w-full bg-transparent border border-transparent hover:border-gray-200 focus:bg-white focus:border-blue-400 rounded px-2 py-1.5 text-gray-700 text-sm focus:ring-2 focus:ring-blue-100 transition-all outline-none" value={row.date} onChange={(e) => { if (index === 0) setSettings(prev => ({ ...prev, startDate: e.target.value })); else updateRow(index, { date: e.target.value }); }} /></td>
                                                            ...
                                                            <td className="p-2"><textarea className="w-full bg-transparent border border-transparent hover:border-gray-200 focus:bg-white focus:border-blue-400 rounded px-2 py-1.5 text-sm focus:ring-2 focus:ring-blue-100 transition-all outline-none resize-y min-h-[4rem] text-gray-600 leading-relaxed" rows={3} value={row.description} onChange={(e) => updateRow(index, { description: e.target.value })} /></td>
                                                            ...
                                                            <td className="p-2 text-right align-top">{shouldMaskPrice(row.transportDetails.some(i => i.sourcePublic)) ? <span className="text-gray-400 font-mono text-sm px-2 py-1.5 block">{maskNumber(row.transportCost, true)}</span> : <input type="number" className="w-full bg-transparent border border-transparent hover:border-gray-200 focus:bg-white focus:border-blue-400 rounded px-2 py-1.5 text-right text-sm focus:ring-2 focus:ring-blue-100 transition-all outline-none text-gray-600 font-medium" value={row.transportCost} onChange={(e) => updateRow(index, { transportCost: parseFloat(e.target.value) || 0, manualCostFlags: { ...row.manualCostFlags, transport: true } })} />}</td>
                                                            <td className="p-2 text-right align-top">{shouldMaskPrice(row.hotelDetails.some(i => i.sourcePublic)) ? <span className="text-gray-400 font-mono text-sm px-2 py-1.5 block">{maskNumber(row.hotelCost, true)}</span> : <input type="number" className="w-full bg-transparent border border-transparent hover:border-gray-200 focus:bg-white focus:border-blue-400 rounded px-2 py-1.5 text-right text-sm focus:ring-2 focus:ring-blue-100 transition-all outline-none text-gray-600 font-medium" value={row.hotelCost} onChange={(e) => updateRow(index, { hotelCost: parseFloat(e.target.value) || 0, manualCostFlags: { ...row.manualCostFlags, hotel: true } })} />}</td>
                                                            <td className="p-2 text-right align-top">{shouldMaskPrice(row.ticketDetails.some(i => i.sourcePublic)) ? <span className="text-gray-400 font-mono text-sm px-2 py-1.5 block">{maskNumber(row.ticketCost, true)}</span> : <input type="number" className="w-full bg-transparent border border-transparent hover:border-gray-200 focus:bg-white focus:border-blue-400 rounded px-2 py-1.5 text-right text-sm focus:ring-2 focus:ring-blue-100 transition-all outline-none text-gray-600 font-medium" value={row.ticketCost} onChange={(e) => updateRow(index, { ticketCost: parseFloat(e.target.value) || 0, manualCostFlags: { ...row.manualCostFlags, ticket: true } })} />}</td>
                                                            <td className="p-2 text-right align-top">{shouldMaskPrice(row.activityDetails.some(i => i.sourcePublic)) ? <span className="text-gray-400 font-mono text-sm px-2 py-1.5 block">{maskNumber(row.activityCost, true)}</span> : <input type="number" className="w-full bg-transparent border border-transparent hover:border-gray-200 focus:bg-white focus:border-blue-400 rounded px-2 py-1.5 text-right text-sm focus:ring-2 focus:ring-blue-100 transition-all outline-none text-gray-600 font-medium" value={row.activityCost} onChange={(e) => updateRow(index, { activityCost: parseFloat(e.target.value) || 0, manualCostFlags: { ...row.manualCostFlags, activity: true } })} />}</td>
                                                            <td className="p-2 text-right align-top">{shouldMaskPrice(row.otherDetails.some(i => i.sourcePublic)) ? <span className="text-gray-400 font-mono text-sm px-2 py-1.5 block">{maskNumber(row.otherCost, true)}</span> : <input type="number" className="w-full bg-transparent border border-transparent hover:border-gray-200 focus:bg-white focus:border-blue-400 rounded px-2 py-1.5 text-right text-sm focus:ring-2 focus:ring-blue-100 transition-all outline-none text-gray-600 font-medium" value={row.otherCost} onChange={(e) => updateRow(index, { otherCost: parseFloat(e.target.value) || 0, manualCostFlags: { ...row.manualCostFlags, other: true } })} />}</td>
